...
**`capture/` 是跨知识库共享的、分类未定的暂存区**。它在顶层而不在某个 `raw/` 里面，
原因是 `raw/` 每个分类各有一个（`kb/<分类>/raw/`）：要把材料放进某个 raw，前提是
已经知道它归哪一类——而「还不知道归哪类」正是 capture 存在的唯一理由。

**判据只有一条：归属分类定了没有。**
- 定了（在电脑前手动放文件、心里清楚它属于哪个分类）→ **直接进 `kb/<分类>/raw/`，
  不经过 capture**。这是主路。
- 没定（微信转发、网页剪藏、会话蒸馏等无人值守入口，agent 不能替你决定分类）→
  落 `capture/`，等你分拣。

`capture/` 里的东西不算知识库内容：不 lint、不链接、不引用。ingest 开始前若 capture
有积压，agent **先提议分拣**（入库→建议分类 / 建议删除+理由），用户一次确认后代为
移动/删除——挑选的劳动归 agent，判断归用户。agent 可以建议归属分类，但不许自建新分类。
（单知识库场景没有归属歧义，这层可以完全不用。）
